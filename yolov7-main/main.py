...
#setting.init()
def calldetect():
    call(["python", "detect3.py", "--weights", "yolov7-tiny.pt", "--conf", "0.25", "--img-size", "640", "--source", "0", "--save-txt"])
def decision_logic():
    global procedure, current_step, gui
    while True:     # prevent calling before initialization
        if gui is not None: break
    x = threading.Thread(target=calldetect)
    x.start()
    ...
    while current_step < len(procedure):
        ...
        #if(content):
        #    if float(content)> thresh:
        #        gui.mark_step_done(DONE)
        if(content):
            #float(content)
            global count
            if int(content)> count:
                for j in range (int(content)-count):
                    gui.mark_step_done(DONE)
                count = int(content)
        #time delay
        #if not (setting.newproc.empty()):
        #    gui.mark_step_done(DONE)
        #    print(detect3.proc)
        #    detect3.proc.get()
        #    setting.newproc.get()
